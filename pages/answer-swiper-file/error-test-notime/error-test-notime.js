...
Page({
  ...
  getListFn() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (names == "我的错题") {
        ...
        param = {
          url: '/api/subjects/getErrorSet',

        }
      } else if (names == "错题练习专区") {
  ...
  incCurrentFn(e) {
    ...
    if (type == 1) {
      ...
      // 判断
      data.user_answer = i;
      trues == 0 ? trues = 2 : '' //因为后台传的是0和1，在这里统一修改下,0改为2，1就是1所以不用改
      if (i == trues) {
        data.isquestion = '1'; //说明用户答对了
        data.istip = false; //是否出现提示 false不出现，true出现
        answer_num = 1 //答题数,答对是1,答错是2
  ...
  delFn(str) {
    var that = this
if( that.data.topicData.length==0)return
    var current = that.data.current

    var id = that.data.topicData[current].id //该题对应的id
    var param = {
      url: '/api/customers/deleteErrorSubject',
      data: {},
    }

    if (str == 'all') {
      // 全部移除
      param.data.subject_id = ''


    } else {

      param.data.subject_id = id
    ...
    app.http(param).then((res) => {
      ...
      if (str == 'all') {
        ...
        that.data.recordnum.err = that.data.recordnum.right = 0
        ...
        that.setData({
          ...
          recordnum: that.data.recordnum
                ...
                // 判断统计的对错数是否变化,对就对题-1，错就错题-1，没答不变
        if (that.data.topicData[current].isquestion == '1') {
          that.data.recordnum.right -= 1
        }  else if (that.data.topicData[current].isquestion == '2') {
          that.data.recordnum.err -= 1;
        }
        // 移除单个 
       that.data.topicData.splice(current, 1)
        if (that.data.list_length == that.data.current + 1) {
          that.setData({
            current: that.data.current - 1,
          })
        }
        that.setData({
          list_length: that.data.list_length - 1,
