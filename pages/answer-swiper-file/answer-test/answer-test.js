...
Page({
  ...
  resetTime(totalTime) {
    ...
    that.data.timer = setInterval(function () {
      ...
      var m = totalTime / 60 < 10 ? ('0' + parseInt(totalTime / 60)) : parseInt(totalTime / 60);
      that.setData({
        data: m + ':' + s
      })
      totalTime--;
  ...
  timeFn(nowTime) {
    var that = this
    var param = {
      url: '/api/index/getPaperTime',

    }
    app.http(param).then((res) => {

      var paperTime = res.data.data.paperTime
      that.setData({
        endTime: paperTime * 1000 + nowTime
      })
      that.resetTime(paperTime)

    }).catch(err => {
      // 具体参考 reject()
      if (err == 'session已更新') {
        this.timeFn(nowTime) //重新调用该函数
    ...
    wx.request({
      url: api + '/api/index/getPaperTime',
      header: {
          ...
          success(res) {
            if (res.confirm) {
              ...
              that.setData({
                ...
                endTime: obj.endTime //真实情况下用到
              })
              that.resetTime(Math.ceil((obj.endTime - nowTime) / 1000))
              setTimeout(function () {
                wx.hideLoading()
              }, 1000)
            } else if (res.cancel) {
  ...
  incCurrentFn(e) {
    ...
    if (type == 1) {
      ...
      // 判断
      data.user_answer = i;
      trues == 0 ? trues = 2 : '' //因为后台传的是0和1，我写的是1和2，在这里统一修改下,0改为2，1就是1所以不用改
      if (i == trues) {
        data.isquestion = '1'; //说明用户答对了
        data.istip = false; //是否出现提示 false不出现，true出现
        answer_num = 1 //答题数,答对是1,答错是2
    ...
    // console.log(data.isquestion,'data.isquestion');
